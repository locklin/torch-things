-- the first loop doesn't quite work yet for the initLen pieces
...
Win = (torch.rand(resSize,2)-0.5):cuda()
...
X = torch.Tensor((1+inSize+resSize),(trainLen-initLen)):fill(0):cuda()
Yt = data:narrow(2,(initLen+1),(testLen- initLen)):cuda()

x = torch.Tensor(resSize,1):fill(0):cuda()
ux = torch.cat(torch.ones(trainLen),data:narrow(2,1,trainLen):t(),2):cuda()
tmp = torch.Tensor(resSize,1):fill(0):cuda()
for i = 1,trainLen do
   u = ux[i]
   tmp:addmm(W,x)
   tmp:addmm(Win,u:t())
   x = x * (1 -1) + torch.tanh(tmp) * a
   tmp:fill(0)
